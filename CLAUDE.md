...
1. **Queued (async) mode** — payload has repo/pr_number/author/files but **no `code`**. Enqueues a job; `BackgroundWorker` later fetches the PR's real diff from GitHub, runs `RiskEngine.assess_resilient`, LLM-enriches the security findings, and posts a structured `AuditOrchestrator.build_report` back via `GitHubClient.upsert_comment`. This is the only path where the **semantic engine is wired in** (the worker constructs `RiskEngine(semantic_service=...)`).
   - **Code fetch (M1, extended in M3):** when the job carries `owner`/`repo`/`pr_number` and GitHub is enabled, `BackgroundWorker._fetch_pr_data` calls `GitHubClient.get_pull_request_data` → `GET /pulls/{n}/files` (paginated), keeping each patch's **added (`+`) lines** as the code to analyze and threading `files`/`file_contents` (patch text) into the job so the worker's `run_full_review` carries documentation findings — full parity with the sync path. The queued payload now threads an **`owner`** key (derived via `_extract_owner`, or split from an `owner/name` repo); without an owner the fetch/post are skipped and the worker analyzes whatever `code` the job holds (usually empty). `GitHubClient._http_json` discards JSON arrays, so PR-file listing uses a separate `_http_json_list` primitive.
